        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                if "database is locked" in str(e).lower():
                    retry_delay = random.uniform(1, 4)
                    logger.warning(f"{log_prefix}: База данных заблокирована для {chat_identifier}. Повтор через {retry_delay:.2f} сек.")
...
async def _status_updater(
    user_id: int, bot: Bot, status_chat_id: int, status_message_id: int,
    stop_event: asyncio.Event, stats_lock: asyncio.Lock
):
    """Периодически обновляет статусное сообщение с текущей статистикой спама."""
    last_text = ""
    ...
    while not stop_event.is_set():
        try:
            async with stats_lock:
                ...
                if not stats:
                    await asyncio.sleep(1) # Ждем, если статистика еще не инициализирована
                    continue

                sent = stats.get('messages', 0)
                errors = stats.get('errors', 0)
                total_sessions = stats.get('sessions_initial_count', 0)
                failed_sessions_count = len(stats.get('failed_sessions', []))
                active_sessions_count = total_sessions - failed_sessions_count

                status_text = (
                    f"<b>🔄 Спам-сессия в процессе...</b>\n\n"
                    f"<b>Статистика:</b>\n"
                    f"  - Отправлено: {sent}\n"
                    f"  - Ошибок: {errors}\n"
                    f"  - Сессий в работе: {active_sessions_count}/{total_sessions}"
                )

            async with ATTACK_STATUS_LOCK:
                is_attack_active = ATTACK_STATUS.get(user_id, False)

            if status_text != last_text or is_attack_active != last_attack_status:
                ...
                last_attack_status = is_attack_active

            await asyncio.sleep(20)

        except asyncio.CancelledError:
            break
        except Exception as e: # Catching general Exception is fine here as it's a background task
            logger.warning(f"UPDATER: Ошибка при обновлении статуса: {e}") # Changed from debug to warning
            await asyncio.sleep(5) # В случае ошибки ждем перед повторной попыткой
    logger.info(f"UPDATER для {user_id}: Остановлен.")


async def spam_loop_task(
    user_id: int, bot: Bot, status_chat_id: int, status_message_id: int, session_limit: Optional[int]
):
    """
    ...
    try:
        ...
        if not active_sessions or chat_count == 0 or not comment_texts:
            ...
            is_attack = ATTACK_STATUS.get(user_id, False) # Check before editing message
            error_text = "❌ Не удалось запустить спам: отсутствуют"
            if not active_sessions: error_text += " активные сессии"
            if chat_count == 0: error_text += " группы"
            if not comment_texts: error_text += " тексты сообщений"
            await bot.edit_message_text(
        ...
        updater_task = asyncio.create_task(_status_updater(user_id, bot, status_chat_id, status_message_id, stop_event, stats_lock), name=f"Updater-{user_id}")
        ...
        for task in pending:
            task.cancel()
        # Также отменяем задачу обновления статуса, которая не была в wait()
        updater_task.cancel()
        
        # Собираем результаты отмененных задач, чтобы дать им время завершиться
        await asyncio.gather(*workers, *pending, updater_task, return_exceptions=True)
        ...
        tasks_to_cancel = [t for t in workers + [updater_task, stop_waiter_task] if t and not t.done()]
        if tasks_to_cancel:
        ...
        # --- Логика автовыхода ---
        if 'ai_settings' in locals() and ai_settings.get("auto_leave_enabled", False) and chat_count > 0 and active_sessions:
            ...
            if not target_chats_to_leave:
                ...
                leave_workers = []
                proxy_cycle = itertools.cycle(proxies) if 'proxies' in locals() and proxies else None
                for phone, s_name in active_sessions.items():
                    assigned_proxy = next(proxy_cycle) if proxy_cycle else None
        ...
        # Удаляем временное сообщение о статусе, чтобы не загромождать чат
        try:
            if 'status_chat_id' in locals() and 'status_message_id' in locals():
                await bot.delete_message(status_chat_id, status_message_id)
        except Exception as e:
            logger.warning(f"DISPATCHER [{user_id}]: Не удалось удалить временное сообщение о статусе: {e}")
