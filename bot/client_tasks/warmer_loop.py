...
from pyrogram.errors import (
    ...
    MsgIdInvalid, ReactionEmpty, UserIsBlocked, PeerIdInvalid, UserPrivacyRestricted
)
...
async def _perform_reaction_action(client, target_channels: list[str], log_prefix: str) -> str:
    ...
    for channel_identifier in target_channels[:5]:
        try:
            ...
            return f"Реакция '{reaction}'С в {chat.title}"
...
async def _perform_dialogue_action(client, partner_peer_id: int, dialogue_phrases: list[str], log_prefix: str) -> str:
    ...
    except (UserIsBlocked, PeerIdInvalid, UserPrivacyRestricted, UserDeactivated) as e:
        logger.warning(f"{log_prefix}: Не удалось отправить сообщение партнеру {partner_peer_id}: {e}. Партнер считается недоступным.")
        raise PartnerUnavailableError(f"Партнер {partner_peer_id} недоступен: {e}") from e
...
async def _warmer_worker(
    ...
    try:
        ...
        while not stop_event.is_set():
            ...
            # --- ИЗМЕНЕНО: Увеличиваем приоритет диалогов ---
            if actions_per_day['dialogue'] > 0 and current_partner_peer_id and dialogue_phrases:
                possible_actions.extend(['dialogue'] * 3) # Добавляем 'dialogue' 3 раза, чтобы повысить шанс выбора
            ...
            if not possible_actions:
                ...
                try:
                ...
                except PartnerUnavailableError as e:
                    ...
                    error_detail_msg = f"Диалог: Партнер недоступен ({e})."

                except UserChannelsTooMuch:
                    error_detail_msg = "Вступление: Аккаунт уже состоит в максимальном количестве каналов."
                    logger.warning(f"{log_prefix}: {error_detail_msg}")

                except ValueError as e:
                    # Отлавливает ошибки типа "Не удалось найти пост", "Список каналов пуст" и т.д.
                    error_detail_msg = f"Действие '{action_type}': {e}"
                    logger.warning(f"{log_prefix}: Ошибка: {error_detail_msg}")

                except FloodWait as e:
                    wait_time = e.value
                    error_detail_msg = f"Общее: FloodWait на {wait_time} сек."
                    logger.warning(f"{log_prefix}: {error_detail_msg}. Сессия будет в муте.")
                    ...
                    # --- ИЗМЕНЕНО: Обработка ReactionsTooMany для совместимости ---
                    if "REACTIONS_TOO_MANY" in str(e):
                        error_detail_msg = "Реакции: Аккаунт временно ограничен Telegram (слишком много реакций)."
                        logger.warning(f"{log_prefix}: {error_detail_msg}")
                        await asyncio.sleep(60) # Короткая пауза, чтобы не "долбить" API
                    else:
                        error_detail_msg = f"Критическая ошибка '{action_type}': {type(e).__name__}"
                        logger.error(f"{log_prefix}: {error_detail_msg}: {e}", exc_info=True)

                # --- ОБНОВЛЕНО: Централизованная запись статистики ---
                if error_detail_msg:
                    async with stats_lock:
                        if user_id in WARMER_STATS:
                            WARMER_STATS[user_id]["errors"] += 1
                            WARMER_STATS[user_id]["error_details"].append(error_detail_msg)
                elif action_result:
                    async with stats_lock:
                        if user_id in WARMER_STATS:
...
async def warmer_loop_task(bot: Bot, user_id: int):
    ...
    try:
        ...
        # Для прогрева используем только Pyrogram сессии
        sessions = await db_manager.get_sessions_by_type(user_id, 'pyrogram')
        stop_event = WARMER_STOP_EVENTS.get(user_id)
        dialogue_phrases = [p.strip() for p in settings.get('dialogue_phrases', '').split(',') if p.strip()]

        WARMER_STATS[user_id] = {
            "actions_done": 0, "errors": 0, "failed_sessions": [],
            "action_details": [], "error_details": [], "active_sessions": len(sessions)
        }
        ...
        error_details = final_stats.get("error_details", [])
        if error_details:
            report_message += "\n<b>🔍 Детали ошибок:</b>\n"
            error_counts = Counter(error_details)
            # Показываем до 5 самых частых ошибок
            for error, count in error_counts.most_common(5):
                report_message += f"  - ({count}x) {html.escape(error)}\n"
            if len(error_counts) > 5:
                report_message += f"  - ... и еще {len(error_counts) - 5} тип(ов) ошибок.\n"
