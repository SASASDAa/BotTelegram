...
def settings_keyboard() -> ReplyKeyboardMarkup:
    return ReplyKeyboardMarkup(
        keyboard=[
            ...
            [KeyboardButton(text="🌐 Прокси"), KeyboardButton(text="👤 Сбор аудитории"), KeyboardButton(text="🗓️ Планировщик")],
            [KeyboardButton(text="🤖 Настройки ИИ"), KeyboardButton(text="⚙️ Общие настройки")],
...
def warmer_settings_keyboard(settings: dict) -> InlineKeyboardMarkup:
    """Создает клавиатуру для настроек прогрева."""
    duration = settings.get('duration_days', 7)
    joins = settings.get('join_channels_per_day', 2)
    reactions = settings.get('send_reactions_per_day', 5)
    target_channels = settings.get('target_channels', '')
    channels_status = f"{target_channels[:20]}..." if target_channels else "не заданы"
    ...
    phrases = settings.get('dialogue_phrases', '')
    phrases_status = f"{phrases[:20]}..." if phrases else "не заданы"
    ...
    buttons = [
        ...
        [InlineKeyboardButton(text=f"📥 Вступлений в день: {joins}", callback_data="warmer_set_joins")],
        [InlineKeyboardButton(text=f"👍 Реакций в день: {reactions}", callback_data="warmer_set_reactions")],
...
def sessions_keyboard_markup(session_statuses_page: list[dict], current_page: int, total_pages: int) -> InlineKeyboardMarkup:
    buttons = [
    ...
    for session in session_statuses_page:
        ...
        # The button's action is to delete. The text should represent the session's state clearly.
        button_text = f"{status} | {phone}"
    ...
    buttons.append([InlineKeyboardButton(text="➕ Добавить вручную", callback_data="add_session_manual")])
    buttons.append([InlineKeyboardButton(text="📤 Загрузить файл(ы)", callback_data="upload_session_file")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
async def general_settings_keyboard(user_id: int) -> InlineKeyboardMarkup:
    """Генерирует клавиатуру для общих настроек."""
    from .database.db_manager import db_manager # Local import to prevent circular dependencies
    ai_settings = await db_manager.get_ai_settings(user_id)
    delay = await db_manager.get_delay(user_id)
    
    persistent_status = "Вкл ✅" if ai_settings.get("persistent_spam", False) else "Выкл ❌"
    auto_leave_status = "Вкл ✅" if ai_settings.get("auto_leave_enabled", False) else "Выкл ❌"
    skip_admins_status = "Вкл ✅" if ai_settings.get("attack_skip_admins", True) else "Выкл ❌"

    buttons = [
        [InlineKeyboardButton(text=f"⏱ Задержка спама: {delay} сек.", callback_data="set_spam_delay")],
        [InlineKeyboardButton(text=f"🔁 Постоянный спам: {persistent_status}", callback_data="toggle_persistent_spam")],
        [InlineKeyboardButton(text=f"📤 Автовыход из групп: {auto_leave_status}", callback_data="toggle_auto_leave")],
        [InlineKeyboardButton(text=f"🚫 Не спамить админам: {skip_admins_status}", callback_data="toggle_attack_skip_admins")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def admin_ban_confirm_keyboard(user_id: int, is_banned: bool) -> InlineKeyboardMarkup:
    ...
    buttons = [
        [action_btn, InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_ban")]
    ]
    return InlineKeyboardMarkup(inline_keyboard=buttons)
...
def attack_menu_keyboard(data: dict) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(text="🔙 Отмена", callback_data="attack_cancel")],
    ]
    return InlineKeyboardMarkup(inline_keyboard=keyboard)


def scraper_menu_keyboard(scraped_count: int) -> InlineKeyboardMarkup:
...
def select_sessions_keyboard(total_sessions: int, task_prefix: str) -> InlineKeyboardMarkup:
    ...
    buttons.append([InlineKeyboardButton(text="🔙 Отмена", callback_data=f"{task_prefix}_sessions_cancel")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)