...
async def add_chats_start(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.edit_text(
        ...
        "Используйте в качестве разделителя <b>|</b> или каждую с новой строки.\nПример: @mygroup | t.me/anothergroup\n"
...
async def find_chats_start(query: CallbackQuery, state: FSMContext):
    ...
    await query.answer()
    await query.message.edit_text(
        "🔍 Введите ключевые слова для поиска ГРУПП (разделитель <b>|</b>).\n"
        "Отправьте /cancel для отмены."
    )
