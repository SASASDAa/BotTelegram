...
from bot.client_tasks.client_manager import (
    ATTACK_STATUS, SPAM_STATUS, validate_user_sessions, SESSION_VALIDATION_CACHE, SESSION_VALIDATION_CACHE_LOCK
)
from bot.database.db_manager import db_manager
from bot.keyboards import sessions_keyboard_markup, settings_keyboard, main_keyboard, shop_keyboard
...
@router.message(F.text == "/cancel", SessionStates.adding_phone, SessionStates.adding_code, SessionStates.adding_password)
...
async def show_sessions_menu(message: Message, user_id: int, page: int = 1):
    """Отправляет или редактирует сообщение с меню сессий."""
    await message.edit_text("🔄 Проверяю сессии, пожалуйста, подождите...")
    try:
        session_statuses = await validate_user_sessions(user_id)
        # Проблемные сессии наверху
        ...
        text = f"<b>📱 Ваши сессии (Страница {page}/{total_pages}):</b>\n\n"
        if not session_statuses:
            text += "Список сессий пуст."
        else:
            text += "Нажмите на сессию, чтобы удалить ее. Проблемные сессии наверху."

        markup = sessions_keyboard_markup(sessions_on_page, page, total_pages)
...
async def refresh_sessions_callback(query: CallbackQuery):
    ...
    await show_sessions_menu(query.message, query.from_user.id, page=current_page)
...
async def delete_session_callback(query: CallbackQuery):
    ...
    # --- ДОБАВЛЕНО: Инвалидация кэша после удаления сессии ---
    # Это гарантирует, что следующий вызов show_sessions_menu получит свежие данные.
    async with SESSION_VALIDATION_CACHE_LOCK:
...
@router.callback_query(F.data == "add_session_manual")
async def add_session_manual_start(query: CallbackQuery, state: FSMContext):
    ...
    await query.message.edit_text(
        "📱 Введите номер телефона (например, +1234567890).\n/cancel для отмены."
    )
    await state.set_state(SessionStates.adding_phone)


@router.message(SessionStates.adding_phone)
...
@router.message(SessionStates.adding_code)
async def add_session_code_received(message: Message, state: FSMContext):
    ...
    try:
        ...
        await db_manager.add_session(user_id, data['phone'], data['session_file_full_path'])
...
@router.message(SessionStates.adding_password)
...
async def upload_session_start(query: CallbackQuery, state: FSMContext):
    ...
    await query.answer()
    await query.message.edit_text(
        "📤 Отправьте один или несколько файлов сессии Pyrogram (.session).\n"
...
async def handle_session_file_upload(message: Message, bot: Bot):
    ...
    try:
        ...
        await db_manager.add_session(user_id, phone_number, session_file_path)
