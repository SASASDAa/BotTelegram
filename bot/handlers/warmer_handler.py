...
from bot.keyboards import warmer_menu_keyboard, warmer_settings_keyboard
...
async def warmer_settings_menu(message: Message, state: FSMContext):
    ...
    sent_message = await message.answer(
        ...
        reply_markup=warmer_settings_keyboard(settings)
    )
    await state.set_state(WarmerStates.menu)
    await state.update_data(menu_message_id=sent_message.message_id)

async def _update_settings_menu_view(bot: Bot, chat_id: int, state: FSMContext):
    ...
    try:
        ...
        markup = warmer_settings_keyboard(settings)
...
@router.callback_query(F.data == "warmer_toggle_inform", WarmerStates.menu)
async def toggle_inform_callback(query: CallbackQuery, state: FSMContext):
    ...
    await _update_settings_menu_view(query.bot, user_id, state)

@router.callback_query(F.data == "warmer_toggle_dialogue", WarmerStates.menu)
...
@router.callback_query(F.data == "warmer_toggle_schedule", WarmerStates.menu)
...
@router.callback_query(F.data.startswith("warmer_set_"), WarmerStates.menu)
