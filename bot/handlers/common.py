...
async def show_status_command(message: Message):
    ...
    # --- ИЗМЕНЕНО: Раздельный подсчет сессий ---
    session_counts = await db_manager.get_session_counts(user_id)
    total_sessions = sum(session_counts.values())
    pyro_count = session_counts.get('pyrogram', 0)
    tele_count = session_counts.get('telethon', 0)
    sessions_status_text = f"Всего: {total_sessions} (Pyrogram: {pyro_count}, Telethon: {tele_count})"

    status_text = (
        f"<b>📊 Статус для {message.from_user.mention_html()}:</b>\n\n"
        f"⭐ Подписка: {'Активна' if sub_conf['active'] else 'Неактивна'}\n"
        f"📱 Аккаунты: {sessions_status_text}\n"
        f"📢 Групп: {chats_count}\n"
